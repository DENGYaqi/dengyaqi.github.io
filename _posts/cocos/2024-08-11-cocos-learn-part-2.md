---
title: CocosCreator学习第二部分 - Cocos Creator
...
- 基础操作
    - 加载资源 : 将资源拖拽进资源管理器
    - 资源 -> 节点 : 将特定资源拖拽进层接管理器变成节点
    - 新增脚本 : 在资源管理器新建脚本
    - 给节点添加脚本功能 : 将脚本拖拽进节点的属性检查器变成节点的功能

- 无限循环背景
    - 两个背景上下移动，看起来像是在前进，创建背景1和背景2
    - 上下移动 : 更改Y坐标
    - 避免不同设备之间的像素不同，速度有关的内容都需要将单位从帧 -> 秒 : update(dt){像素 * dt}
    - 背景2移动到顶部，让背景1置于背景2上方。

- 玩家
    - 飞机与手指的坐标重合
    - 函数传参带有问号表示不用传
    - 攻击 计时器 : 每个节点都可以调用一个计时器 schedule(每隔多长时间调用第匿名函数, 多长时间来一次, 重复几次 不传则一直重复, 等多久开始第一次调用)，例如 this.schedule(()=>{//创建敌人}, 0.5, 100, 0)，第一个敌人瞬间产生，每隔0.5秒创建一个敌人，直到创建到100个为止。
    - 关联子弹预设体 : 创建预设体代码变量、节点回拖至资源管理器形成预设体、再将预设体拖入玩家脚本的变量内。

- 子弹
    - 创建子弹的预设体
    - 子弹的速度 : 一秒一屏的速度 = 800 也就是画布高度
