...
红黑树是一种自平衡的二叉搜索树，它保证了在最坏情况下的操作时间复杂度为 O(log n)。
  ...
  - 概念解释
    - 节点颜色 : 每个节点都是红色或黑色。
    - 根节点是黑色。空节点（即树的叶子）被认为是黑色。新插入的节点默认是红色。
    - 红色节点的规则 : 一个红色节点的两个子节点不能同时为红色。
    - 黑色高度 : 从任意节点到其所有叶子节点的路径中，必须包含相同数量的黑色节点(即黑色高度相同)。
   
  - 实现原理
    - 自平衡 : 红黑树通过节点的颜色和旋转操作保持平衡，使得树的高度始终保持在 O(log n) 范围内。

  - 操作
    - 新增 : 标准插入(二叉搜索树的方式，红色) - 修正红黑树性质(重新着色或旋转) - 旋转操作(左旋或右旋)，
