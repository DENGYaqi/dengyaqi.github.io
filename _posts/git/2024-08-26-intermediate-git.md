...
## 进阶操作
1. 配置和使用多个远程仓库
添加新的远程仓库upstream : `git remote add upstream <第二个远程仓库的 URL>`、查看所有远程仓库 : `git remote -v`、将upstream的某个分支合并到本地分支 : `git merge upstream/分支名`、推送代码到执行的远程仓库 : `git push origin 分支名`。
