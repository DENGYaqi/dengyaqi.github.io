...
把消息发送给broker中的某个topic，并打开kafka的客户端 :
...
可以去到路径`/usr/local/kafka/kafka_2.11-2.4.1/data/kafka-logs`内，可以看到50个consumer-offsets分区，通过消费者自己去维护消费到哪个位置。重点查看test-0，进入test-0文件内查看结构，`xxx.index`索引文件，`xxx.timeindex`时间索引文件，`xxx.log`**存放消息文件**。
